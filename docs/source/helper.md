...
Available options for `kind` are: `standard` | `empty` | `alignment` | `validation` | `symmetry` | `feature` | `density` | `membrane` | `feature-density` | `motif` | `nakb` | `glygen` | `default-assembly`
...
#### default-assembly
Show the default assembly of the structure.
...
.. function:: get_volume(url_obj, isovalues, entryId, isBinary, isLazy=True)
   ...
   :param url_obj: The volume source, generated by the helper function ``parse_url()``.
                   The URL must point to a volume file in a supported format.
   :type url_obj: dict
...
Each isosurface is described by a dict that mirrors molstar's `VolumeIsovalueInfo` structure. The available keys are:

- `type` (str): How `value` is interpreted. Either `'relative'` (a multiple of the map's standard deviation, i.e. sigma) or `'absolute'` (an absolute value in the map's units).
- `value` (float): The isovalue at which to draw the surface.
- `color` (int): The hex color of the surface, e.g. `0x3362B2`.
- `alpha` (float, optional): The opacity of the surface, from `0.0` to `1.0`. (default: `1.0`)
- `volumeIndex` (int, optional): Which map within the source to draw this surface from, used when a single source contains multiple maps. For an X-ray density response, `0` is typically the `2Fo-Fc` map and `1` the `Fo-Fc` difference map. (default: `0`)

### Loading a single map

For a single density map (e.g. a `ccp4` file), pass one isovalue and one `entryId`:
...
mol = parse_url('https://www.ebi.ac.uk/pdbe/entry-files/download/5a1a.cif')
source = parse_url('https://www.ebi.ac.uk/emdb/volume_server/em/EMD-2984/cell?detail=6', fmt='dscif')
volume = get_volume(
   source,
...
When the source serves more than one map, such as the RCSB volume server returning both the `2Fo-Fc` and `Fo-Fc` maps, pass a list of isovalues and use `volumeIndex` to assign each surface to the correct map. The `entryId` should then be a list labeling each map:

```py
from dash_molstar.helpers import parse_url, get_volume

source = parse_url(
   'https://maps.rcsb.org/x-ray/1tqn/box/-41.696,-62.869,-43.855/7.288,14.016,20.119?detail=3',
   fmt='dscif'
)
isovalues = [
   {'type': 'relative', 'value': 1.5, 'color': 0x3362B2, 'alpha': 0.3},                    # 2Fo-Fc
...
```py
data = [
   parse_url('https://files.rcsb.org/download/1TQN.pdb'),
   volume,
]
```

:::{tip}
To expose molstar's built-in volume streaming controls in the UI, enable them through the viewer's ``layout`` property:
