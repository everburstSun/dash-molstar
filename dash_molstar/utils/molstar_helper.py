...
def get_targets(chain, residue=None, atom=None, auth=False):
    ...
    if residue is not None:
        ...
        for res in residue:
            if type(res) == int: target.chains[0].add_residue(res)
            elif type(res) == str:
                try:
                    num = eval(res)
                    target.chains[0].add_residue(num)
                except:
                    num = eval(res[:-1])
                    ins = res[-1]
                    target.chains[0].add_residue(num, ins_code=ins)
    return target
...
def create_component(label, targets, representation=Representation()):
    ...
    return {
        'label': label,
        'targets': [t.to_dict() if type(t)==Target else t for t in targets],
        'representation': [r.to_dict() if type(r)==Representation else r for r in representation]
    }
