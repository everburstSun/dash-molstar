...
def get_screenshot(filename, params, crop=None):
    """
    Create a screenshot of the current viewport in molstar viewer. The returned value can be passed to `screenshot` parameter of callbacks.

    Parameters
    ----------
    `filename` — str
        The name of the screenshot file to be downloaded.
    `params` — dict
        The parameters for taking screenshot, generated by `ViewportScreenshotHelperParams` in molstar.
    `crop` — dict (optional)
        The cropping parameters for the screenshot, with keys `x`, `y`, `width`, and `height`. (default: `None`)

    Returns
    -------
    `dict`
        The screenshot data for callbacks.
    """
    data = {
        'filename': filename,
        'params': params.to_dict() if isinstance(params, Screenshot) else params
    }
    if params.crop and crop is None:
        data['crop'] = params.crop
    if crop:
        data['crop'] = crop
    return data

def set_camera(camera=None, duration=0):
    """
    Create a camera operation. If a list of camera operations were provided to callback, 
    they will be executed sequentially with the specified duration for each transformation.

    Parameters
    ----------
    `camera` — Camera (optional)
        The `Camera` instance to move to. If not provided, the camera will be reset (default: `None`)
    `duration` — int (optional)
        Transformation duration in milliseconds (default: `0`)

    Returns
    -------
    `dict`
        Data to provided to the `camera` parameter of callbacks
    """
    data = {'duration': duration}
    if camera is not None:
        if not isinstance(camera, Camera):
            raise TypeError("The camera parameter should be a valid Camera object. Use helper class `Camera` to generate the camera data.")
...
def get_box(min_xyz=(0,0,0), max_xyz=(1,1,1), radius=0.1, label="Bounding Box", color='red', opacity=1.0):
    ...
    return shapes.create_box(min_xyz, max_xyz, radius, label, color, opacity)
def get_sphere(center=(0,0,0), radius=1.0, label="Sphere", color='blue', opacity=1.0, detail=6):
    ...
    return shapes.create_sphere(center, radius, label, color, opacity, detail)
def get_cylinder(start=(0,0,0), end=(1,1,1), radius=0.1, label="Cylinder", color='yellow', opacity=1.0, dashed=False, dash_segments='auto'):
    ...
    return shapes.create_cylinder(start, end, radius, label, color, opacity, dashed, dash_segments)